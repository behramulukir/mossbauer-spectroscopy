...
class Mossbauer(Simulator):

    """
    Class to simulate sequential data according to the mossbauer model. Used in cases where observations are needed at several design times.
    """

    def __init__(self, true_theta):

        """
        true_theta: true theta value, scalar
        S0: starting population of death model
        """

        super(Mossbauer, self).__init__(true_theta)
    def summary(self, Y):
    ...
    def generate_data(self, d, p):
        ...
        """
        d: design points at which to simulate data (multi-dimensional array); it needs to be ordered
        p: model parameter (scalar)
        """
        gamma_square = 0.1
        h = 1.0
        h_0 = 1.0
        y_val = lambda theta, control: (h_0 - ((h*gamma_square) / ((theta - control) * (theta - control) + gamma_square)))
